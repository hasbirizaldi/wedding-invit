...
// import ThemeSelect from "../../components/others/ThemeSelect";
...
const EditAdmin = () => {
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        await axios.post(`https://www.brewokode.site/api/admin/wedd/${id}`, formData, {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoadingpage(true);
        const { data } = await axios.get(`https://www.brewokode.site/api/admin/wedd/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  return (
    <div className="bg-gray-200/80 lg:px-4 md:px-2 px-1 py-5 rounded shadow-ku font-noto">
      <h1 className="text-pink-950 text-2xl font-bold ml-3 font-one">Edit Undangan</h1>
      <div className="overflow-x-auto">
        <form onSubmit={handleSubmit} className="space-y-2">
          ...
          <div className="grid lg:grid-cols-2 grid-cols-1 gap-1">
            <div className="bg-white lg:w-[99%] md:w-[99%] w-[98%] px-3 lg:px-6 pt-6 pb-8 rounded shadow-ku mx-auto">
              ...
              <div className="mb-3">
                ...
                <input type="file" name="foto_mp" onChange={handleFileChange} className={`w-full cursor-pointer px-3 py-1.5 border border-gray-300 rounded shadow-sm focus:ring-pink-400 focus:border-pink-400  outline-none text-sm `} />
                {errors.foto_mp && <span className="text-red-500 text-xs">{errors.foto_mp}</span>}

                {(fotoMp || form.foto_mp) && <img src={fotoMp ? URL.createObjectURL(fotoMp) : `https://www.brewokode.site/storage/${form.foto_mp}`} alt="Foto Mempelai Pria" className="w-26 h-26 object-cover rounded mt-2" />}
              </div>
              <div className="mb-3">
                <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <input type="file" name="foto_mw" onChange={handleFileChange} className="w-full px-3 cursor-pointer py-1.5 border border-gray-300 rounded shadow-sm focus:ring-pink-400 focus:border-pink-400  outline-none text-sm" />
                {errors.foto_mw && <span className="text-red-500 text-xs">{errors.foto_mw}</span>}
                {(fotoMw || form.foto_mp) && <img src={fotoMw ? URL.createObjectURL(fotoMw) : `https://www.brewokode.site/storage/${form.foto_mw}`} alt="Foto Mempelai Pria" className="w-26 h-26 object-cover rounded mt-2" />}
              </div>
              <div className="mb-3">
                <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <input
                  ...
                  className="w-full px-3  cursor-pointer py-1.5 border border-gray-300 rounded shadow-sm focus:ring-pink-400 focus:border-pink-400  outline-none text-sm"
                  multiple
                />
                ...
                <div className="grid lg:grid-cols-4 md:grid-cols-3 grid-cols-2 gap-2 mt-2">
                  {previewGalleries.length > 0
                    ? previewGalleries.map((src, i) => <img key={i} src={src} className="w-32 h-32 object-cover rounded border border-gray-200" />)
                    : form.galleries?.map((path, i) => <img key={i} src={`https://www.brewokode.site/storage/${path}`} className="w-32 h-32 object-cover rounded border border-gray-200" />)}
                </div>
              </div>
            </div>
            <div className="bg-white lg:w-[99%] md:w-[99%] w-[98%] px-3 lg:px-6 pt-6 pb-8 rounded shadow-ku mx-auto">
          ...
          <button type="submit" className="mt-4 w-full bg-pink-950 text-white py-2 px-4 rounded font-semibold hover:bg-pink-900 cursor-pointer transition">
            {loading ? "Memproses Data...." : "Update Undangan"}
          </button>
        </form>
      </div>
    </div>
